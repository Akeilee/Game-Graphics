...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	...
	buildingTex = SOIL_load_OGL_texture(TEXTUREDIR "barren reds.JPG", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS);
	buildingBump = SOIL_load_OGL_texture(TEXTUREDIR "barren redsdot3.jpg", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS);
	...
	for (int i = 0; i < 10; ++i) {
		...
		s->SetTransform(Matrix4::Translation(Vector3(rand() % 2501 + 250, 110, 500 + 300 * i)));
	...
	//light = new Light(heightmapSize * Vector3(0.75f, 5.5f, 0.0f), Vector4(1, 1, 1, 1), heightmapSize.x * 100.0f);

	//shadow work but textue no
	camera = new Camera(0.0f, 220.0f, heightmapSize * Vector3(0.5f, 5.0f, 0.0f));
	light = new Light(heightmapSize * Vector3(-0.50f, 2.10f, -0.30f), Vector4(1, 1, 1, 1), heightmapSize.x * 10.0f);
	//light->SetColour(Vector4(0.50f, 0.5f, 0.1f, 1.0f));
...
void Renderer::DrawFloorShadow() {
	...
	viewMatrix = Matrix4::BuildViewMatrix(light->GetPosition() * 0.5, Vector3(50,-30,50));
	projMatrix = Matrix4::Perspective(1.0f, 15000.0f, (float)width / (float)height, 45.0f);
	shadowMatrix = projMatrix * viewMatrix; // used later
...
void Renderer::DrawHeightmap2() {
	...
	//viewMatrix = camera->BuildViewMatrix();
	//viewMatrix = Matrix4::BuildViewMatrix(light->GetPosition() * 0.5, heightMap->GetHeightmapSize() * Vector3(1, 1, 1));
	//projMatrix = Matrix4::Perspective(1.0f, 15000.0f, (float)width / (float)height, 45.0f);
	//shadowMatrix = projMatrix * viewMatrix; // used later
	...
	//shadowMatrix.ToIdentity();  //doesn't work
